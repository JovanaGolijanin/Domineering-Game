...
def novaFunkcija():
    print("Ovo su svi igraci koji su uspeli da pobede racunar")
    response = requests.get('http://localhost:5173/VratiIgrace')
    data = response.json()
    for item in data:
     print("Ime:", item['ime'], "|  BrojIndeksa:", item['brojIndeksa'], "|  BrojPobeda:", item['brojPobeda'])
...
def ulogujteSe():
    ...
    response = requests.post(f'http://localhost:5173/DodajIgraca?ime={ime}&brojIndeksa={brIndeksa}')
    if response.ok:
        print("Uspesno ste kreirali nalog, mozete da pocnete sa igrom")
    else:
...
def igracPobedio():
    ...
    response = requests.put(f'http://localhost:5173/DodajPobedu?brojIndeksa={brIndeksa}')
    data = response.json()
...
def igracIzgubio():
    ...
    response = requests.put(f'http://localhost:5173/DodajPoraz?brojIndeksa={brIndeksa}')
    data = response.json()
...
def logovanje():
    print("Unesite broj indeksa kako bi ste igrali, ako prvi put igrate morate da napravite nalog")
    global brIndeksa
    brIndeksa = int(sys.stdin.readline())
    response = requests.get(f'http://localhost:5173/VratiIgraca?brojIndeksa={brIndeksa}')
    data = response.json()

    if(data == []):
        print("Vas indeks ne postoji, molimo vas da napravite nalog")
        ulogujteSe()
    else:
...
def init():
    ...
    while (unos < 2):
        print("Uneli ste nevalidnu vrednost. Unesite broj veći od 1.")
        unos = int(sys.stdin.readline())
    brojVrsta = unos
...
def koIgraPrvi():
    ...
    while (unos not in range(0,2)):
        print("Uneli ste nevalidnu vrednost. Unesite O ili 1.")
        unos = int(sys.stdin.readline())
    igraPrvi = unos
...
def minimax(pocetno_stanje, dubina: int, alpha: int, beta: int, na_potezu):
    ...
    if na_potezu=='X':
        ...
        for potez in validni_potezi_X(pocetno_stanje):
            ...
            vrednost, _ = minimax(sledece_stanje, dubina - 1, alpha, beta,'O')
        ...
        for potez in valdini_potezi_O(pocetno_stanje):
            ...
            vrednost, _ = minimax(sledece_stanje, dubina - 1, alpha, beta,'X')
            if vrednost < najbolja_vrednost:
                najbolja_vrednost = vrednost
                najbolji_potez = potez
...
def start():
    ...
    while True:
        if(krajIgre(matrica,naPotezu)):
            pobedio=""
            if(naPotezu=='X'):  #Ukoliko je covek prvi i izgubi ovo ce se pozvati
                pobedio='O'
                igracIzgubio()
            else:
                pobedio='X'
        ...
        vrednost, potez = minimax(pocetno_stanje, 5, -math.inf, +math.inf,naPotezu)
